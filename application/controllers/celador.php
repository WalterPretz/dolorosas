...
function __construct(){
	...
	function crearCelador(){
		...
		if ( (!empty($_POST['nombre'])) || (!empty($_POST['apellido'])) || (!empty($_POST['nacimiento'])) || (!empty($_POST['edad'])) || (!empty($_POST['genero'])) || (!empty($_POST['numero'])) || (!empty($_POST['direccion'])) || (!empty($_POST['muni'])) || (!empty($_POST['tipo'])) || (!empty($_POST['ofrenda'])) || (!empty($_POST['descripcion'])) ) {
			...
			$col_id_colaborador = $this->Celador_model->crearCelador($data['cui'], $data['muni_id_muni'], $data['tipo'], $data['anios'], $data['ofrenda'], $data['descripcion'], $data['cargar']);
	...
	function listado(){
		...
		if ($this->session->ROL == 'Directora') {
			...
			$data['arr'] = $this->Celador_model->listadoCelador($inicio, $fin);
	...
	function listadoColaborador(){
		...
		if ($this->session->ROL == 'Directora') {
			...
			$data['arr'] = $this->Celador_model->listadoColaborador($inicio, $fin);
	...
	function coordinador(){
		...
		if ($this->session->ROL == 'Directora') {
			...
			$data['arr'] = $this->Celador_model->listadoCoordinador($inicio, $fin);
	...
	function inscripcionhonor(){
		...
		$data['arr'] = $this->Celador_model->listadoCeladorHonor($inicio, $fin);
		$data['arr1'] = $this->Celador_model->listadoCeladorHonor1($inicio, $fin);
