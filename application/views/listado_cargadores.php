       ...
       <?php foreach ($arr as $a){ ?>
          <tr>
            ...
            <td class="text-center oculalimprimir"><a class='btn btn-success btn-sm botones' onchange="return llenar("href='<?=$base_url?>/cargador/detalle_cargador/<?php echo $a['id_cargador']; ?>') id="id_colaborador"><i class="fa fa-info-circle"></i></a></td>
              <?php if ($this->session->ROL == 'Directora') { ?>
            <td class="text-center oculalimprimir"><a class='btn btn-warning btn-sm botones' data-bs-toggle="modal" data-bs-target="#editarDatosCargador" 
...
<!-- Modal -->
<div class="modal fade" id="editarDatosCargador" data-bs-backdrop="static" data-bs-keyboard="false" tabindex="-1" aria-labelledby="staticBackdropLabel" aria-hidden="true">
  <div class="modal-dialog modal-lg modal-dialog-centered">
...
<script type="text/javascript">
  function obdatosIdPerso(id_colaborador) {
      ...
      $.ajax({
          data: datos,
          url: '<?=$base_url?>/cargador/buscarpersona',
          type: 'POST',
          beforeSend: function(){},
  ...
  $('#deleteuss').submit(function(e){
    e.preventDefault();
        $.ajax({
            url: '<?=$base_url?>/cargador/changepersona',
            type: "POST",
            async: true,
        ...
        $.ajax({
            ...
            success: function(response){
              ...
              if (response == 'true') {
                ...
                Swal.fire({
                title: 'Se a actualizado con éxito a la cargadora.',
                icon: 'success',
                confirmButtonText: '<i class="fa fa-thumbs-up"></i> Ok!',
                 }).then((result) => {
                  if (result.isConfirmed) {
