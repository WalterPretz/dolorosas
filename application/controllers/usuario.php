...
function __construct(){
	...
	function crear_user(){
		...
		if(!empty($_POST['nombre']) || !empty($_POST['apellido']) || !empty($_POST['cui']) || !empty($_POST['telefono']) || !empty($_POST['direccion']) || !empty($_POST['usuario']) || !empty($_POST['rol']) || !empty($_POST['clave']) || !empty($_POST['clave2']) ){
			//print_r($_POST);
			...
			if ($data['clave'] != $data['clave2']) {
				...
				$crear = $this->Usuario_model->crearUsuario($data['cui'], $data['cargo'], $data['usuario'], $data['clave'], $data['rol'], $data['email']);
				$persona = $this->Usuario_model->crearUsuarioPersona($data['nombre'], $data['apellido'], $data['direccion'], $crear);
				$tel = $this->Usuario_model->crearUsuarioTelefono($data['numero1'], $data['numero2'], $crear);
