...
class Usuario_model extends CI_Model{
	...
	function crearUsuarioPersona($nombre, $apellido, $direccion, $cod){
		$sql = "INSERT INTO persona(nombre, apellido, direccion, us_id_usuario, col_id_colaborador, car_id_cargador)
				VALUES (?, ?, ?, ?, ?, ?)";

		$col_id_colaborador = 1;
		$car_id_cargador = 1;
		$valores = array($nombre, $apellido, $direccion, $cod, $col_id_colaborador, $car_id_cargador);
		$dbres = $this->db->query($sql, $valores);

		return $dbres;
	}

	function crearUsuarioTelefono($numero1, $numero2, $cod){
		$sql = "INSERT INTO telefono(numero1, numero2, us_id_usuario, col_id_colaborador, car_id_cargador)
				VALUES (?, ?, ?, ?, ?)";

		$col_id_colaborador = 1;
		$car_id_cargador = 1;
		$valores = array($numero1, $numero2, $cod, $col_id_colaborador, $car_id_cargador);
		$dbres = $this->db->query($sql, $valores);
